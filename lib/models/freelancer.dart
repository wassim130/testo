...
class FreelancerModel {
  ...
  factory FreelancerModel.fromMap(Map<String, dynamic> map) {
    return FreelancerModel(
      ...
      stats_clients: map['stats_clients'] ?? 3,
      stat_rati: map['stat_rati'] ?? 4,
      // tools: List<TechnologyToolsModel>.from(
      //     map['tools']?.map((x) => TechnologyToolsModel.fromMap(x)) ?? []),
      // technologies: List<TechnologyModel>.from(
      //     map['technologies']?.map((x) => TechnologyModel.fromMap(x)) ?? []),
      //  tools: map['tools'] != null
      //     ? List<TechnologyToolsModel>.from(map['tools']?.map((x) => TechnologyToolsModel.fromMap(x)))
      //     : [],
      // technologies: map['technologies'] != null
      //     ? List<TechnologyModel>.from(map['technologies']?.map((x) => TechnologyModel.fromMap(x)))
      //     : [],
      tools: map['tools'] != null && map['tools'].isNotEmpty
          // ? List<TechnologyToolsModel>.from(
          // map['tools']?.map((x) => TechnologyToolsModel.fromMap(x)))
          ? List<TechnologyToolsModel>.from(map['tools']!.map((item) {
              return TechnologyToolsModel.fromMap(item);
            }))
          : [],
      technologies:
          map['technologies'] != null && map['technologies'].isNotEmpty
